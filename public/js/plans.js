  ...
  grid.innerHTML = plans
    ...
    <div class="col-md-4">
      <div class="plan-card p-4 glass-card h-100">
        ...
        <div class="mt-3">
          <button class="btn btn-outline-accent w-100" onclick="openInvestModal('${
            p._id
          }', '${p.title}', ${p.minAmount})">Invest</button>
        </div>
      </div>
    </div>
  `
...
function openInvestModal(id, title, minAmount) {
  ...
  document.getElementById(
    "min-amount-hint"
  ).textContent = `Minimum ${minAmount} USDT`;
  const investModal = new bootstrap.Modal(
